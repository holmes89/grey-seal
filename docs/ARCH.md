...
- **`"ollama:<model>"`** — an in-process tool-calling run with no repo and no PR: `lib/repo/ollamarunner` runs a bounded loop (≤8 model turns, 8-minute per-turn timeout, 40-minute whole-run ceiling) against a local Ollama model (`lib/repo/ollamatools`, `/api/chat` with tools) whose tools come from an MCP server over Streamable HTTP (`lib/repo/mcpclient` → `agent.ToolProvider`). It emits `agent.message` heartbeats at each phase (connecting, resolving projects, per model turn) so `StreamAgentRun` shows progress even while a slow first turn runs. When a model prints its tool call into the message body instead of the `tool_calls` field (common with small local models such as `qwen2.5-coder:7b`), the runner parses those `{"name","arguments"}` objects back out and runs them. Used for orchestration tasks such as decomposing a design into DRAFT rabbit tickets. Gated on `REMORA_MCP_URL`; model from `OLLAMA_AGENT_CHAT_MODEL`.
