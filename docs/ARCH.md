...
- **`"ollama:<model>"`** — an in-process run with no repo and no PR that decomposes a software design into DRAFT rabbit tickets: `lib/repo/ollamarunner` resolves the project list from the MCP server (`list_projects`), then makes **one** schema-constrained model call (`lib/repo/ollamatools` `ChatJSON` → `/api/chat` with Ollama's `format` JSON schema, no tools) that returns the whole plan `{project_uuid, tickets[]}`. When `RunAgentTaskRequest.project_uuid` is set (the UI knows which project the design belongs to) the runner pins that project, validates it against the list, and the model is asked only for `{tickets[]}`; otherwise the model picks a project by name and the runner validates its `project_uuid` against the resolved list. One corrective retry on an unusable plan, then the runner calls the `create_ticket` MCP tool itself for each ticket with `draft: true` — the model never drives the tool loop, since small local models are unreliable at multi-turn tool-calling but solid at schema-constrained JSON. Tools reach it over Streamable HTTP (`lib/repo/mcpclient` → `agent.ToolProvider`). Emits `agent.message` heartbeats at each phase (connecting, resolving projects, asking the model, planning N tickets) so `StreamAgentRun` shows progress. Per-call timeout 10m, whole-run ceiling 25m. Gated on `REMORA_MCP_URL`; model from `OLLAMA_AGENT_CHAT_MODEL`.
